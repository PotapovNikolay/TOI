...
cv.imwrite('pic/rotation.png',image_dst)
...
M1 = np.float32([ [2, 0  , 0],
              [0,   2, 0],
              [0,   0,   1] ])
image_scale = cv.warpPerspective(image,M1,(int(w*2),int(h*2)))
cv.imshow('Масштаб', image_scale)
cv.imwrite('pic/scale.png',image_scale)
#сдвиг
...
cv.imwrite('/pic/sdvig.png',sheared_img)
cv.waitKey(0)